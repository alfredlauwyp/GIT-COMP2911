 ...
 * DualPoint interface.
 * 
 * Provides the class for a "node" that consists of  
 *  two points - a point "from" and a point "to". These
 *  two pairs of coordinates can be accessed. The distance
 *  between the cartesian coordinates of the origin and destination
 *  points can be accessed. The external distance between
 *  this JobPoint's "destination" coordinates and another
 *  JobPoint's "origin" coordinates can also be accessed.
 * 
...
public class JobPoint implements DualPoint
{
	...
	{
		return "["+this.getFromX()+","+this.getFromY()+"]->["+this.getToX()+","+this.getToY()+"]";
	}
	
	public boolean isSamePoint()
	{
		if (this.getFromX() == this.getToX() && this.getFromY() == this.getToY())
		{
			return true;
		}
		return false;
